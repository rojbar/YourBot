...
help.data.addStringOption(option => option.setName('query')
    .setDescription('What you want to search')
    .setRequired(true));
help.execute = async (interaction) => {
    await interaction.deferReply();
    axios.get(`https://en.wikipedia.org/wiki/${interaction.options.getString('query').trim().replace(/ /g, '_')}`).then(res => {
